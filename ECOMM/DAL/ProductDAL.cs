...
namespace DAL
{
    public class ProductDAL
    {
      ...
      public static string conString = ConfigurationManager.ConnectionStrings["ECOMMDBConnectionString"].ToString();
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(conString))
                {
                    if (con.State == ConnectionState.Closed)
                        con.Open();
                    string query = "INSERT INTO flowers (Id,Title, Description, Image, UnitPrice, Quantity) " +
                        "VALUES (@productId, @picture, @Description, @Image, @UnitPrice, @Quantity, @Likes)";
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(conString))
                {
                    ...
                    string query = "UPDATE products SET Title=@Title , Description=@Description, " +
                        "Image=@Image, UnitPrice=@UnitPrice, Quantity=@Quantity, Likes=@Likes " +
                        "WHERE Id=@Id";
                    SqlCommand cmd = new SqlCommand(query, con);
                    cmd.Parameters.Add(new SqlParameter("@Id", product.ID));
                    cmd.Parameters.Add(new SqlParameter("@Title", product.Title));
